...
class TS_CPC(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        concat = torch.cat((x,x_fft), dim=1)
      

        z = self.encoder(concat)  # Apply the encoder
...
# The outputs from both temporal and spectral encoders can then be fused (e.g., via concatenation or bilinear transformation) before the NCE loss is computed.
    ...
    def predict(self, x, hidden):
        ...
        concat = torch.cat((x, x_fft), dim=1)   # concatenation along channel dimension
        

        z = self.encoder(concat)
        z = z.transpose(1, 2)
